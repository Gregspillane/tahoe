...
class GoogleSpeechClient:
    ...
    def __init__(
        ...
        try:
            if api_key:
                ...
                logger.info(f"GoogleSpeechClient initialized with API key authentication")
            elif credentials_path and os.path.exists(credentials_path):
                credentials = service_account.Credentials.from_service_account_file(credentials_path)
                self.client = speech.SpeechClient(credentials=credentials)
                logger.info(f"GoogleSpeechClient initialized with service account file")
            else:
                # Use default credentials (environment variables or metadata server)
                # This will use the API key set in environment
                self.client = speech.SpeechClient()
                logger.info(f"GoogleSpeechClient initialized with default/API key credentials")
            
            logger.info(f"GoogleSpeechClient model: {model}, language: {language_code}")
            
            # Test the client with a simple operation to catch auth issues early
            try:
                # This will fail gracefully if API is not enabled
                config = speech.RecognitionConfig(
                    encoding=speech.RecognitionConfig.AudioEncoding.LINEAR16,
                    sample_rate_hertz=16000,
                    language_code=self.language_code,
                )
                logger.info("Google Speech client configuration validated")
            except Exception as test_e:
                logger.warning(f"Google Speech API may not be enabled: {test_e}")
    ...
    async def _transcribe_sync(self, audio_file_path: str, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Google API error in sync transcription: {e}")
