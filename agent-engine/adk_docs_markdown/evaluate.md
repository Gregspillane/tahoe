...
1. **In-order match:** Requires the correct actions in the correct order, allows for extra actions.
1. **Any-order match:** Requires the correct actions in any order, allows for extra actions.
1. **Precision:** Measures the relevance/correctness of predicted actions.
1. **Recall:** Measures how many essential actions are captured in the prediction.
1. **Single-tool use:** Checks for the inclusion of a specific action.
...
1. Eval set data is developed and maintained manually and used in ADK eval CLI A
