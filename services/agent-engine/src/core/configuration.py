...
class ConfigurationLoader:
    ...
    def _apply_spec_overrides(self, spec: Dict[str, Any]):
        ...
        logger.info(
            f"Applying config overrides from {metadata.get('name', 'unknown')} for {environment}"
        )
    ...
    def validate_environment(self) -> Dict[str, Any]:
        ...
        if self._config.is_production:
            ...
            if (
                not self._config.adk.gemini_api_key
                or self._config.adk.gemini_api_key.startswith("CHANGE_THIS_")
            ):
