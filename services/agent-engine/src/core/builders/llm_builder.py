...
class LlmAgentBuilder(AgentBuilder):
    ...
    def build(self, spec: AgentSpec, context: AgentContext) -> LlmAgent:
        ...
        if not self.validate_spec(spec):
            raise ValueError(
                f"Invalid LLM agent specification: {spec.metadata.get('name')}"
            )
        ...
        instruction = self._build_instruction(
            agent_spec["instruction_template"], context
        )
        ...
        # Get model configuration (single primary model only - fail fast approach)
        model_config = agent_spec.get("model", {})
        model = model_config.get("primary", "gemini-2.5-flash-lite")

        # Get model parameters and prepare generate_content_config
        model_params = model_config.get("parameters", {})
        additional_params = agent_spec.get("parameters", {})
        
        # Combine model and agent parameters
        all_params = {**model_params, **additional_params}
        
        # Prepare generate_content_config if model parameters are specified
        ...
        if all_params:
            try:
                ...
                # Map common parameters to GenerateContentConfig
                if "temperature" in all_params:
                    config_params["temperature"] = all_params["temperature"]
                if "max_tokens" in all_params:
                    config_params["max_output_tokens"] = all_params["max_tokens"]
                if "max_output_tokens" in all_params:
                    config_params["max_output_tokens"] = all_params["max_output_tokens"]
                
                if config_params:
                    generate_config = types.GenerateContentConfig(**config_params)
            except ImportError:
                logger.warning("google.genai.types not available, model parameters will be ignored")
            except Exception as e:
                logger.warning(f"Could not create GenerateContentConfig: {e}")

        # Create LlmAgent with proper ADK patterns
        agent_kwargs = {
        ...
        # Add generate_content_config if available
        if generate_config:
            agent_kwargs["generate_content_config"] = generate_config
    ...
    def _build_instruction(self, template: str, context: AgentContext) -> str:
        ...
        for var in variables:
            if var in context.variables:
                ...
                logger.warning(
                    f"Variable {var} not found in context, using placeholder"
                )
    ...
    def _evaluate_condition(self, condition: str, context: AgentContext) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Complex condition '{condition}' not supported, defaulting to False"
            )
            return False
