...
class ADKConfig(BaseSettings):
    ...
    fallback_models: List[str] = Field(
        default=["gemini-2.5-pro", "gemini-2.5-flash"], env="ADK_FALLBACK_MODELS"
    )
...
class ObservabilityConfig(BaseSettings):
    ...
    log_format: str = Field(
        default="json" if IS_PRODUCTION else "text", env="LOG_FORMAT"
    )
...
class AgentEngineConfig(BaseSettings):
    ...
    max_concurrent_executions: int = Field(
        default=10, env="AGENT_ENGINE_MAX_CONCURRENT"
    )
