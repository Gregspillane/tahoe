...
class DatabaseService:
    ...
    This service manages the database connection lifecycle and provides
    methods for all database operations. It ensures proper connection
    management and cleanup to prevent connection leaks.
    """

    def __init__(self):
        """Initialize database service with connection tracking."""
        self.prisma = Prisma()
        ...
        logger.info(
            f"DatabaseService initialized with backend={self.session_backend}, namespace={self.redis_namespace}"
        )

    async def connect(self) -> None:
        """Connect to database with connection tracking.

        This method is idempotent - multiple calls will not create multiple connections.
        """
        if not self._connected:
            try:
                ...
                logger.info(
                    f"Database connected (connection #{self._connection_count})"
                )
    ...
    async def disconnect(self) -> None:
        ...
        This method ensures all pending operations are completed before disconnecting.
        """
        if self._connected:
            try:
                ...
                self._connected = False
                logger.info(
                    f"Database disconnected (was connection #{self._connection_count})"
                )
            except Exception as e:
                logger.error(f"Error during database disconnect: {e}")
                # Even on error, mark as disconnected to allow retry
                self._connected = False
                raise
        else:
            logger.debug("Database already disconnected, skipping disconnect")

    async def ensure_connected(self) -> None:
        """Ensure database is connected, reconnecting if necessary.

        This method should be called before any database operation to ensure
        ...
        if not self._connected:
            ...
            # Optionally, we could add a ping here to verify connection is still alive
            try:
                # Simple query to verify connection
                await self.prisma.session.count()
            except Exception as e:
    ...
    async def get_connection(self):
        ...
        This ensures the database is connected before operations and
        maintains the connection for reuse (no disconnect on exit).
        """
        try:
            await self.ensure_connected()
            yield self
        finally:
            # Keep connection alive for reuse - disconnect handled by shutdown event
            pass

    async def transaction(self, operations: Callable) -> Any:
        """Execute operations within a database transaction.
        
        Simple transaction wrapper for critical multi-step operations.
        If any operation fails, all changes are rolled back.
        
        Args:
            operations: Async function that takes a transaction client
        
        Returns:
            Result from the operations function
            
        Example:
            async def critical_operation(tx):
                await tx.session.update(...)
                await tx.execution.create(...)
                return result
                
            result = await db.transaction(critical_operation)
        """
        await self.ensure_connected()
    ...
    async def create_session(
        ...
        session = await self.prisma.session.create(
            data={
                ...
                "state": json.dumps(state) if state else json.dumps({}),
                "backend": backend or self.session_backend,  # Track backend
                "metadata": json.dumps(metadata or {"source": "adk", "namespace": self.redis_namespace}),
            }
        )
    ...
    async def get_session(self, session_id: str) -> Optional[Any]:
        ...
        session = await self.prisma.session.find_unique(
            where={"session_id": session_id}, include={"executions": True}
        )
        return session
    ...
    async def update_session_state(
        ...
        if session:
            # Handle both dict and string states from Prisma
            ...
            updated_session = await self.prisma.session.update(
                where={"session_id": session_id}, data={"state": json.dumps(current_state)}
            )
    ...
    async def update_execution(
        ...
        execution = await self.prisma.execution.update(
            where={"id": execution_id}, data=data
        )
    ...
    async def get_execution(self, execution_id: str) -> Optional[Any]:
        ...
        execution = await self.prisma.execution.find_unique(
            where={"id": execution_id}, include={"results": True}
        )
        return execution
    ...
    async def create_result(
        ...
        # Only include metadata if provided (it's optional)
        if metadata is not None:
            create_data["metadata"] = json.dumps(metadata)
        
        result = await self.prisma.result.create(data=create_data)
    ...
    async def get_tool(self, name: str) -> Optional[Any]:
        ...
        tool = await self.prisma.toolregistry.find_unique(where={"name": name})
        return tool
    ...
    async def save_configuration_version(
        ...
        """Save configuration version with transaction for atomicity."""
        
        async def _save_config(tx):
            # Deactivate previous versions
            await tx.configurationversion.update_many(
                where={"type": type, "name": name, "active": True}, 
                data={"active": False}
            )

            # Create new active version
            import json
            config = await tx.configurationversion.create(
                data={
                    "type": type,
                    "name": name,
                    "version": version,
        ...
        # Execute within transaction - rolls back if any step fails
        return await self.transaction(_save_config)

    async def get_configuration_version(
        self, type: str, name: str, version: Optional[str] = None
    ) -> Optional[Any]:
        ...
        if version:
            config = await self.prisma.configurationversion.find_first(
                where={"type": type, "name": name, "version": version}
            )
        else:
            # Get active version
            config = await self.prisma.configurationversion.find_first(
