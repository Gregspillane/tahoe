...
class ConfigurationVersionService:
    ...
    def rollback_to_version(
        self, kind: str, name: str, checksum: str
    ...
    def compare_versions(self, checksum1: str, checksum2: str) -> Dict[str, Any]:
        ...
        differences = {"checksum1": checksum1, "checksum2": checksum2, "changes": []}
        ...
        for key in added_keys:
            differences["changes"].append(
                {"type": "added", "key": key, "value": spec2[key]}
            )

        for key in removed_keys:
            differences["changes"].append(
                {"type": "removed", "key": key, "value": spec1[key]}
            )
...
def track_specification_version(
    spec: Dict[str, Any], service: Optional[ConfigurationVersionService] = None
) -> str:
